...
// R looks for R_init_{dll_name} (= R_init_basemapper from basemapper.dll).
// extendr generates R_init_basemapper_extendr instead, so we bridge here to
// ensure the extendr runtime is initialized when R loads the DLL.
//
// On Windows (both MSVC and GNU targets), extendr's global variable imports from
// R.dll (e.g. R_NilValue) require a GNU import library (libR.dll.a) that marks
// those symbols as DATA exports — see .r-lib/R_gnu.def. This is generated by
// scripts/build-r-importlib.ps1 and must be regenerated when R is upgraded.
#[no_mangle]
pub extern "C" fn R_init_basemapper(dll_info: *mut std::os::raw::c_void) {
    R_init_basemapper_extendr(dll_info as *mut _);
}
